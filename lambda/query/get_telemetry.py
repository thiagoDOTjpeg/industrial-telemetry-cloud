...
def lambda_handler(event, context):
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute("SELECT machine_id, temperature, status, created_at, vibration_level FROM telemetry WHERE created_at > NOW() - INTERVAL '1 hour' ORDER BY created_at DESC LIMIT 50")
        rows = cur.fetchall()
